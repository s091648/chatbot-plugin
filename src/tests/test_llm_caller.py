"""Tests for LLM call paths (_call_llm and _call_gemini)."""
    ...
    with patch("httpx.AsyncClient", return_value=mock_client):
        with patch.object(settings, "gemini_api_key", "fake-key"):
            with patch.object(settings, "gemini_model", "gemini-test"):
                service = ToolboxService(AsyncMock())
                result = await service._call_gemini("sys", "prompt")
                assert result == "Hi from Gemini"


@pytest.mark.asyncio
async def test_call_gemini_api_error():
    """_call_gemini raises HTTPException when Gemini returns non-200."""
    mock_resp = MagicMock()
    ...
    with patch("httpx.AsyncClient", return_value=mock_client):
        with patch.object(settings, "gemini_api_key", "fake-key"):
            ...
            with pytest.raises(HTTPException) as exc:
                await service._call_gemini("sys", "prompt")
...
async def test_call_gemini_malformed_response():
    """_call_gemini raises HTTPException on unexpected response shape."""
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    ...
    with patch("httpx.AsyncClient", return_value=mock_client):
        with patch.object(settings, "gemini_api_key", "fake-key"):
            ...
            with pytest.raises(HTTPException) as exc:
                await service._call_gemini("sys", "prompt")
            assert "Unexpected Gemini response" in exc.value.detail
    ...
    with patch("anthropic.AsyncAnthropic", return_value=mock_client):
        with patch.object(settings, "llm_api_key", "sk-real"):
            with patch.object(settings, "llm_model", "claude-test"):
                service = ToolboxService(AsyncMock())
                result = await service._call_llm("some context", "question?")
            ...
            with patch.object(settings, "llm_api_key", "sk-real", create=True):
                with patch.object(settings, "gemini_api_key", "fake-key", create=True):
                    with patch.object(settings, "llm_model", "claude-test"):
                        with patch.object(settings, "gemini_model", "gemini-test"):
                            service = ToolboxService(AsyncMock())
                            result = await service._call_llm("ctx", "q?")
